...
const ChartsPage = ({ data }) => {
  ...
  const handleDownloadPDF = () => {
    ...
    doc.autoTable({
      head: [['Date', 'Max Temperature (°C)', 'Temperature (°C)', 'Min Temperature (°C)']],
      body: data.days.map(day => [day.datetime, day.tempmax, day.temp, day.tempmin])
    });
  ...
  const handleDownloadPrecipPDF = () => {
    ...
    doc.autoTable({
      head: [['Date', 'Precipitation', 'Precipitation Cover', 'Precipitation Probability']],
      body: data.days.map(day => [day.datetime, day.precip , day.precipcover, day.precipprob])
    });
  ...
  return (
    <div>
      ...
      <div className="charts-container">
        ...
        <button className='pfdbutton'  onClick={(e) => {
        e.preventDefault();
        handleDownloadPDF();
      }}
       >  Download PDF </button>
